<template>
  <main class="categories-page">
    <section class="categories-section">
      <div class="categories-layout">
        ...
        <aside class="categories-sidebar" aria-label="分类筛选">
          <div class="categories-container" id="categoriesContainer">
            ...
            <div class="categories-wheel-row">
              <div v-for="level in MAX_LEVELS" :key="level - 1" class="categories-wheel-wrap">
                <div class="categories-wheel-label">
                  {{ wheelLabels[level - 1] }}
                </div>
                <div class="categories-wheel" :data-level="level - 1" :style="wheelStyle" role="listbox" tabindex="0"
                  :aria-label="`${wheelLabels[level - 1]}筛选`" @scroll="handleWheelScroll(level - 1, $event)"
                  @click="handleWheelItemClick(level - 1, $event)" @keydown="handleWheelKeydown(level - 1, $event)">
                  <div v-for="(tag, idx) in getTagsForLevel(level - 1)" :key="idx" class="wheel-item" :class="{
          ...
          <div class="archive-filter-panel" aria-label="博客类型筛选">
            <div class="archive-filter-toggle" role="radiogroup" aria-label="博客类型筛选">
              <button v-for="filter in filterOptions" :key="filter.key" class="archive-filter-btn"
                :class="{ active: currentFilter === filter.key }" :data-filter="filter.key" role="radio"
        ...
        <div class="blog-list" id="blogList" ref="blogListRef">
          ...
          <div v-for="(blog, index) in filteredBlogs" :key="blog.id" class="blog-item"
            ...
            <router-link class="blog-link" :to="`/blog/${blog.id}`">
              ...
              <div class="blog-right">
                <div class="blog-tags">
                  <span v-for="(tag, idx) in (blog.tags || [])" :key="idx" class="blog-tag" :data-level="idx"
    ...
    <!-- 移动端 FAB -->
    <button id="categoriesFab" class="categories-filter-fab" aria-label="打开筛选"
      :aria-hidden="isModalOpen ? 'true' : 'false'" @click="openModal">
      <i class="fas fa-sliders" aria-hidden="true"></i>
    </button>

    <!-- 移动端弹窗 -->
    <div id="categoriesModal" class="categories-filter-modal" :class="{
      'open': isModalOpen && !isModalClosing,
      'closing': isModalClosing,
      'open-prep': isModalPrepping
    }" :aria-hidden="!isModalOpen" @click="handleBackdropClick">
